...
function ShowWaktu() {
  ...
  for (var i = 1; i <= 6; i++) {
    ...
    wsolat.push({ masa: TimeToTime(masa), waktu: wname[i] }); // Format 12 jam
    wsolat24.push({
  ...
  DateTime.wnxt = {
    waktu: wname[wnxt],
    masa: TimeToTime(nxtw), // Format 12 jam
    timesec: parseInt(nxtw ?? -1) * 100,
  };
  DateTime.wnxt24 = {
    waktu: wname[wnxt],
    masa: Digit2(parseInt(nxtw / 100)) + ":" + Digit2(nxtw % 100), // Format 24 jam
    timesec: parseInt(nxtw ?? -1) * 100,
  };
