...
function Time12Hour(txt, hour24) {
  return `${TimeToTime(txt)} ${parseInt(hour24 / 100) >= 12 ? "PM" : "AM"}`;
}
function ValToTime(dd) {
  dd = parseInt(dd);
  return "" + Digit2(parseInt(dd / 100)) + ":" + Digit2(dd % 100);
}
...
function HijriDate() {
  ...
  var SetF = 30 - DayH; // first month days
  var DatP = 1,
    BitP = 0;
...
function ShowWaktu() {
  ...
  for (var i = 1; i <= 6; i++) {
    var masa = sysData.wdata[DateTime.days][i]; // Waktu solat asli dalam format 24 jam
    if (i < 6) {
      min1 = TimeToMin(masa);
    ...
    if ([2].includes(i)) allowAzan.push({ start: min1 - 5, end: min1 + 5 });

    // Simpan waktu dalam 12 jam dan 24 jam
    wsolat.push({ masa: Time12Hour(masa, masa), waktu: wname[i] }); // Format 12 jam
    wsolat24.push({ masa: ValToTime(masa), waktu: wname[i] }); // Format 24 jam
    // masa: Digit2(parseInt(masa / 100)) + ":" + Digit2(masa % 100),
  }
  ...
  // masa: Digit2(parseInt(nxtw / 100)) + ":" + Digit2(nxtw % 100),

  // Logik untuk menentukan azan dibenarkan
  const curMin = mins;
  DateTime.allowAzan = !allowAzan.some(
